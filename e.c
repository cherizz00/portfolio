...
<title>Asteria — AI Commerce Demo</title>
<style>
...
<header class="header">
  <div class="container header-inner">
    ...
    <div class="search" id="searchBar">
      ...
      <input id="q" placeholder="Search products… try “white sneakers”, “denim”, or drop an image below">
...
<!-- Assistant -->
<div class="fab" id="chatFab" title="Shopping assistant">🤖</div>
<div class="drawer" id="chatDrawer" style="display:none">
  ...
  <div class="content" id="chatContent">
    <div class="msg bot"><div class="bubble">Hi! Ask for “running shoes under $100” or “compare denim jackets vs hoodies”. I will recommend from the current catalog.</div></div>
  </div>
  <div class="chat-input">
    <input id="chatInput" placeholder="Ask about size, fit, budget, or product comparisons…">
...
function renderCart(){
  ...
  SESSION.cart.forEach(i=>{
    ...
    row.innerHTML = `<div class="small">${i.title} × ${i.qty}</div>
...
function compareItems(q){
  // crude: if “vs” present, pick top 2 relevant
  ...
  const [a,b] = ranked;
  return `Compare:
- ${a.title}: ${priceFmt(a.price)} — ${a.desc}
- ${b.title}: ${priceFmt(b.price)} — ${b.desc}
...
function onChatSend(){
  ...
  if(recs.length){
    const lines = recs.map(p=>`• ${p.title} — ${priceFmt(p.price)}`).join('<br>');
